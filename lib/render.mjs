...
// On Vercel we download the full Chromium pack (binary + shared libs, incl.
// libnss3) at runtime — bundling the libs into the function is unreliable, this
// isn't. Version MUST match @sparticuz/chromium-min in package.json.
...
async function getBrowser() {
  ...
  browserP = (async () => {
    if (onServerless) {
      return puppeteer.launch({ args: chromium.args, executablePath: await chromium.executablePath(CHROMIUM_PACK), headless: chromium.headless });
    }
